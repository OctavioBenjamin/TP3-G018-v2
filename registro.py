...
def filtrar_lenguaje(proyectos, filtro):
    ...
    if not encontrados:
        print(f"No se han encontrado proyectos de {lenguajes[filtro]}")
    

def proyecto_año(proyectos, num_filtro, v_conteo):
    
    for proyecto in proyectos:
        if proyecto.numero == num_filtro:
...
def mostrar_por_año(v_conteo):
    for i in range(len(v_conteo)):
        if v_conteo[i] > 0:
            if i < 10:
                print(f"Proyectos 200{i} es de: {v_conteo[i]}.")
            else:
                print(f"Proyectos 20{i} es de: {v_conteo[i]}.")

def mostrar_cambios(v_conteo):
    mayor = 0
    for i in range(len(v_conteo)):
        if v_conteo[i] > mayor:
            mayor = v_conteo[i]
            # if i < 10:
            #     print('La cantidad de proyectos modificados del año 200'+str(i),'es de',mayor)
            # else:
            #     print('La cantidad de proyectos modificados del año 20'+str(i),'es de',mayor)

    for i in range(len(v_conteo)):
        if v_conteo[i] == mayor:
            if i < 10:
                print(f"Año 200{i}: {mayor}")
            else:
                print(f"Año 20{i}: {mayor}")
